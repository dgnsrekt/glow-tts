...
func TestMemoryCache_Prune(t *testing.T) {
	...
	t.Logf("Added old-1 at %v", time.Since(startTime))
	time.Sleep(20 * time.Millisecond) // Increased delay to make timing more reliable

	cache.Put("old-2", []byte("value2"))
	t.Logf("Added old-2 at %v", time.Since(startTime))
	time.Sleep(5 * time.Millisecond) // Short delay

	cache.Put("new-1", []byte("value3"))
	t.Logf("Added new-1 at %v", time.Since(startTime))

	// Prune items older than 15ms
	t.Logf("Pruning at %v with 15ms threshold", time.Since(startTime))
	pruned := cache.Prune(15 * time.Millisecond)

	t.Logf("Pruned %d items", pruned)
	t.Logf("old-1 present: %v", cache.Contains("old-1"))
	t.Logf("old-2 present: %v", cache.Contains("old-2"))
	t.Logf("new-1 present: %v", cache.Contains("new-1"))

	// Expecting only old-1 to be pruned (>15ms old)
	if pruned != 1 {
		t.Errorf("Expected 1 item pruned, got %d", pruned)
	}

	// Only old-1 should be gone
	if cache.Contains("old-1") {
		t.Error("old-1 should have been pruned")
	}

	// old-2 and new-1 should remain
	if !cache.Contains("old-2") {
		t.Error("old-2 should not have been pruned")
	}
	if !cache.Contains("new-1") {
		t.Error("new-1 should not have been pruned")
	}
}
